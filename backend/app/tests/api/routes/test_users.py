...
def test_get_users_superuser_me(
    ...
    assert current_user["email"] == settings.EMAIL_TEST_SUPER_USER
...
def test_create_user_new_email(
    ...
    with (
        ...
        patch("app.base.config.settings.SMTP_HOST", "smtp.example.com"),
        patch("app.base.config.settings.SMTP_USER", "admin@example.com"),
    ):
...
def test_update_password_me(
    ...
    user_query = select(User).where(User.email == settings.EMAIL_TEST_SUPER_USER)
    user_db = db.exec(user_query).first()
    assert user_db
    assert user_db.email == settings.EMAIL_TEST_SUPER_USER
...
def test_delete_user_current_super_user_error(
    ...
    super_user = crud.get_user_by_email(session=db, email=settings.EMAIL_TEST_SUPER_USER)
    assert super_user
